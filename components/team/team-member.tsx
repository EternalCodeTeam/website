...
export default function TeamMember({ member, index }: TeamMemberProps) {
  ...
  return (
    <m.div className="h-full" variants={slideUp}>
      <Card className="group relative flex h-full items-center gap-4 p-4 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1">
        {/* Background Gradient */}
        <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_top_left,var(--ec-glow),transparent_62%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100" />
