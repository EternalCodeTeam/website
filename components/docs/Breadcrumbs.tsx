...
export const Breadcrumbs: React.FC<BreadcrumbsProps> = React.memo(
  ({ currentPath, className = "" }) => {
    ...
    return (
      <nav
        className={`mb-4 flex items-center space-x-2 text-sm text-gray-500 dark:text-gray-400 ${className}`}
        aria-label="Breadcrumb navigation"
      >
        <Link
          href="/"
          className="no-underline transition-colors duration-200 hover:text-gray-900 dark:hover:text-white"
