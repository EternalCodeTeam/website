...
| `disableElytraUsage` | boolean | `true` | Prevent elytra activation during combat |
| `disableElytraOnDamage` | boolean | `true` | Disable active elytra when taking damage |
...
Configure PlaceholderAPI placeholder values.

```yaml
placeholders:
  # Text when player is in combat
  isInCombatFormattedTrue: "In Combat"
  
  # Text when player is not in combat
...
| :--- | :--- |
| `%eternalcombat_opponent%` | Name of combat opponent |
| `%eternalcombat_opponent_health%` | Opponent's health (00.00 format) |
| `%eternalcombat_remaining_seconds%` | Seconds until combat ends |
| `%eternalcombat_remaining_millis%` | Milliseconds until combat ends |
| `%eternalcombat_isInCombat_formatted%` | Formatted combat status text |

<Callout type="note" title="Empty Placeholders">
  If a player is not in combat, placeholders return an empty string. Opponent placeholders also return empty if combat wasn't triggered by another player.
</Callout>
...
Available colors for BossBar: `PINK`, `BLUE`, `RED`, `GREEN`, `YELLOW`, `PURPLE`, `WHITE`

### Common Message Placeholders

| Placeholder | Description |
| :--- | :--- |
| `{TIME}` | Remaining combat time |
| `{PLAYER}` | Player name |
| `{PERMISSION}` | Required permission |

<Callout type="tip" title="MiniMessage Format">
  EternalCombat uses MiniMessage for rich text formatting:
  
  - Colors: `<red>`, `<green>`, `<#ff5555>`
  - Gradients: `<gradient:red:yellow>text</gradient>`
  - Formatting: `<bold>`, `<italic>`, `<underlined>`
  
  [Learn more about MiniMessage](https://docs.advntr.dev/minimessage/format.html)
</Callout>
