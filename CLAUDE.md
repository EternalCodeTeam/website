...
EternalCode.pl website - A Next.js 16 website built with React 19, featuring documentation for open-source projects,
built with the App Router, MDX content, and Framer Motion animations.
...
Shared AI skills/context live in **`.ai/skills/`** (committed to Git).  
...
4. **Accessibility**: All animations check `useReducedMotion()` hook and disable animations when user prefers reduced
   motion
...
This repository uses Claude Code Agent Skills to enforce best practices and guidelines. Skills are located in
`.agents/skills/` and activated via symlinks in `.agent/skills/`.
...
Skills are automatically available to Claude Code and will be referenced during code generation and review. To manually
invoke a skill review, you can ask Claude Code to review specific files or patterns against these guidelines.
